...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if '/ban_user' in msgText:
            ...
            if isadmin:
                try:
                    user = str(msgText).split(' ')[1]
                    if user == username:
                        bot.sendMessage(update.message.chat.id,'😂Usted no puede banearse a si mismo')
                        return
                    jdb.remove(user)
                    jdb.save()
        ...
        if '/crypt' in msgText:
            ...
            bot.sendMessage(update.message.chat.id, f'👤Encryptado:\n{proxy}')
            return
        ...
        if '/start' in msgText:
            start_msg = '😉Hola!\n'
            start_msg+= '🎩Desarrollador y editor🖋: @Zeta30 \n'
        ...
        #         bot.editMessageText(message,'❌Error y Causas🧐\n1-Revise su Cuenta\n2-Servidor Desabilitado: '+client.path)
            ...
            #    bot_token = '5554579401:AAFNrfVg-nW6Zqzhnw57UrdgqydTRKCclmg'
